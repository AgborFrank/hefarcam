...
trait ImageStore
{
    ...
    public function imageResize($image, $height = null ,$lenght = null)
    {
        if(isset($image)){
            ...
            if ($image->extension() == "svg") {
                ...
                $image_extention = str_replace('image/','',Image::make($image)->mime());
                if($height != null && $lenght != null ){
                    $img = Image::make($image)->resize(200, $lenght);
                }else{
                    $img = Image::make($image);
                }
                $img_name = 'uploads/images/'.$current_date.'/'.uniqid().'.'.$image_extention;
...
//            $file->storeAs('/uploads/file/'.$current_date.'/', $file_name);
    ...
    public static function saveKycFile(UploadedFile $file)
    {
        if(isset($file)){
            $current_date  = Carbon::now()->format('d-m-Y');

            if(!\File::isDirectory('uploads/file/kyc'.$current_date)){
                \File::makeDirectory('uploads/file/kyc/'.$current_date, 0777, true, true);
            }
            $file_name = uniqid().'.'.$file->extension();
//            $file->storeAs('/uploads/file/'.$current_date.'/', $file_name);
            $s = Storage::disk('custom')->put('file/'.$current_date, $file);
